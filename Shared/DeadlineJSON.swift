...
typealias DeadlineJSONList = [PurpleDeadlineJSON]

struct PurpleDeadlineJSON: Codable {
...
extension PurpleDeadlineJSON {
    init?(data: Data) {
        guard let me = try? JSONDecoder().decode(PurpleDeadlineJSON.self, from: data) else { return nil }
        self = me
    }
